				...
				enum power_supply_property psp, union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		if (!battery_meter_get_battery_current_sign()) //discharge
...
void mt_battery_GetBatteryData(void)
{
	...
	battery_log(BAT_LOG_CRTI,
		    "[kernel]AvgVbat %d,bat_vol %d, Vsense %d,  AvgI %d, I %d, VChr %d, AvgT %d, T %d, ZCV %d, CHR_Type %d\n",
		    BMT_status.bat_vol, bat_vol, Vsense, BMT_status.ICharging, ICharging,
...
{
	unsigned int charging_enable = 0;
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
		...
		if (BMT_status.charger_exist && BMT_status.charger_type == NONSTANDARD_CHARGER)
		{
			ktime = ktime_set(NON_STD_CHARGER_CHECK, 0);	/* 5s */
