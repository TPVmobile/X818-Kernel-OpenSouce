...
/* Modify by TPV/WillHuang, 20151225, { */
#define CONTROL_AF_POWER 1 // 0
/* Modify by TPV/WillHuang, 20151225, } */
	 ...
	 {SENSOR_DRVNAME_OV8858_MIPI_RAW,
	  ...
	  {SENSOR_DRVNAME_IMX214_MIPI_RAW,
	  {
	   ...
	   {RST, Vol_High, 0}
         ...
         {SENSOR_DRVNAME_S5K3P3SX2ND_MIPI_RAW,
          {
           {SensorMCLK, Vol_High, 0},
           {DOVDD, Vol_1800, 0},
           {AVDD, Vol_2800, 0},
...
int mtkcam_gpio_set(int PinIdx, int PwrType, int Val)
{
	...
	case AVDD:
		...
		///else if (Val == 1 && !IS_ERR(cam0_rst_h))    //Mask by TPV/WillHuang, 20160307
		else if (Val == 1 && !IS_ERR(cam_ldo_vcama_h))      //Modify by TPV/WillHuang, 20160307
...
#if CONTROL_AF_POWER
		if (pinSetIdx == 0) {   //Added by TPV/WillHuang, 20160223
...
{
	if (pwInfo.PowerType == AVDD) {
		if (pinSetIdx == 1) { // for font camera. TPV/WillHuang 20160223 modified
