...
/*20160415, Eric Lin add for fixing DEF S2 CN-520/DEF S2 CN-430 {*/
static DEFINE_MUTEX(gsensor_i2c_mutex);
/*20160415, Eric Lin add for fixing DEF S2 CN-520/DEF S2 CN-430 }*/
...
static int bma_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
	...
	if (!client)
	{
		mutex_unlock(&gsensor_i2c_mutex);
		return -EINVAL;
	}
...
static int BMA250_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	memset(databuf, 0, sizeof(u8) * 10);
/*Eric Lin, 20160420, Fix G-sensor issue{*/
...
/*20160415, Eric Lin add for debugging use later {*/
static ssize_t show_reg_dump_value(struct device_driver *ddri, char *buf)
{
	ssize_t len = 0;
	struct i2c_client *client = bma250_i2c_client;
	u8 databuf[2]={0x00};
	...
	for(i=0;i<0x40;i++){
		hwmsen_read_byte(client,i,databuf);
...
/*20160415, Eric Lin add for debugging use later {*/
static DRIVER_ATTR(dump, S_IRUGO, show_reg_dump_value, NULL);
/*20160415, Eric Lin add for debugging use later }*/
...
/*20160415, Eric Lin add for debugging use later {*/	
	&driver_attr_dump,/*dump register values*/
/*20160415, Eric Lin add for debugging use later }*/	
};
