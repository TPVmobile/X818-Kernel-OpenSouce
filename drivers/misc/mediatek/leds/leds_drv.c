...
#endif
//Add massage led note
static ssize_t test_set(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	/*
	if(strncmp(buf, "1", 1) == 0) {
		brightness_set_pmic(1,255,0);
	} else if(strncmp(buf, "2", 1) == 0) {
		brightness_set_pmic(2,255,0);
	} else if(strncmp(buf, "4", 1) == 0) {
		brightness_set_pmic(3,255,0);
	} else {
		brightness_set_pmic(3,0,0);
		brightness_set_pmic(2,0,0);
		brightness_set_pmic(1,0,0);
	}
	*/
	
		if(strncmp(buf, "4", 1) == 0) {
		//brightness_set_pmic(1,255,0);    //Red
		mt65xx_leds_brightness_set(MT65XX_LED_TYPE_RED, LED_FULL); 
	} else if(strncmp(buf, "2", 1) == 0) {
		//brightness_set_pmic(2,255,0);    //Green
		mt65xx_leds_brightness_set(MT65XX_LED_TYPE_GREEN, LED_FULL); 
	} else if(strncmp(buf, "1", 1) == 0) {
		//brightness_set_pmic(3,255,0);   //Blue
...
static ssize_t test_get(struct device *dev, struct device_attribute *attr, char *buf)
{
	int flag = 0;
	...
	int blue = 0;
		...
		green = pmic_get_register_value(PMIC_ISINK_CH1_EN);
		blue = pmic_get_register_value(PMIC_ISINK_CH0_EN);
		red = pmic_get_register_value (PMIC_ISINK_CH4_EN);
...
static DEVICE_ATTR(test,0660, test_get, test_set);
//Add massage led note
/****************************************************************************
 * driver functions
 ***************************************************************************/
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	int i;
	int ret;/* rc; */
	struct device *dev = &pdev->dev; //Add massage led note
	struct cust_mt65xx_led *cust_led_list = mt_get_cust_led_list();
	#ifdef CONFIG_BACKLIGHT_SUPPORT_LP8557
...
#endif
	device_create_file(dev, &dev_attr_test); //Add massage led note
	return 0;
...
static int mt65xx_leds_remove(struct platform_device *pdev)
{
	...
	device_remove_file(dev,&dev_attr_test); //Add massage led note
	return 0;
}
