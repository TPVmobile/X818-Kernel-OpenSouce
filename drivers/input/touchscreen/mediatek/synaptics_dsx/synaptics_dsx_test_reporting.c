...
static ssize_t synaptics_rmi4_f54_raw_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	int retval;
	unsigned char data;
	...
	if (f54->status != STATUS_BUSY) {
		data=0x03;
		f54->report_type = (enum f54_report_types)data;
		retval = f54->fn_ptr->write(rmi4_data,
				f54->data_base_addr,
				&data,
				sizeof(data));
	...
	for (ii = 0; ii < f54->report_size; ii += 18) {
	ptr+=sprintf(ptr,"%d,%d,%d,%d,%d,%d,%d,%d,%d,",
			(f54->report_data[ii + 1]<<8)+f54->report_data[ii],
			(f54->report_data[ii + 3]<<8)+f54->report_data[ii + 2],
			(f54->report_data[ii + 5]<<8)+f54->report_data[ii + 4],
			(f54->report_data[ii + 7]<<8)+f54->report_data[ii + 6],
			(f54->report_data[ii + 9]<<8)+f54->report_data[ii + 8],
			(f54->report_data[ii + 11]<<8)+f54->report_data[ii + 10],
			(f54->report_data[ii + 13]<<8)+f54->report_data[ii + 12],
			(f54->report_data[ii + 15]<<8)+f54->report_data[ii + 14],
			(f54->report_data[ii + 17]<<8)+f54->report_data[ii + 16]
			);
	}
	len=ptr-buf;
	// kfree(page);
