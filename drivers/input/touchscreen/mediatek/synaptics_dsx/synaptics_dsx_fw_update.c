...
#endif
#include "SynaImage.h"
 #include "SynaImage_boe.h"
  #include "SynaImage_truly.h"
extern unsigned char synaptics_touch_type;
/*
#ifdef DO_STARTUP_FW_UPDATE
//#ifdef CONFIG_FB
...
static ssize_t fwu_sysfs_read_config_show(struct device *dev,
...
static struct device_attribute attrs[] = {
	...
	__ATTR(readconfig, S_IRUGO,
			fwu_sysfs_read_config_show,
			fwu_sysfs_read_config_store),
	__ATTR(configarea, S_IRUGO,
			synaptics_rmi4_show_error,
			fwu_sysfs_config_area_store),
	__ATTR(imagename, S_IRUGO,
			synaptics_rmi4_show_error,
...
static unsigned int g_image_config_id=0;
static unsigned int g_device_config_id=0;
static unsigned int g_image_fw_id=0;
static unsigned int g_device_fw_id=0;
...
static enum flash_area fwu_go_nogo(void)
{
	...
	g_image_fw_id=image_fw_id;
	g_device_fw_id=device_fw_id;
	...
	device_config_id = be_to_uint(config_id + retval - 4);
	dev_info(&fwu->rmi4_data->i2c_client->dev,
	...
	/* Get image config ID */
	image_config_id = be_to_uint(&fwu->img.ui_config.data[0]);
	dev_info(&fwu->rmi4_data->i2c_client->dev,
	...
	g_image_config_id=image_config_id;
	g_device_config_id=device_config_id;
...
static int fwu_start_reflash(void)
{
	...
	if (retval < 0)
		goto exit;
	printk("synaptics fw_bl_version=%d,img_bl_version=%d \n",fwu->bl_version,fwu->img.bl_version);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	printk("@@@ make sure code comes here\n");
	if(synaptics_touch_type==0)
		synaptics_fw_updater(SynaFirmware);
	else if(synaptics_touch_type==1)
		synaptics_fw_updater(SynaFirmware_boe);
	else if(synaptics_touch_type==2)
		synaptics_fw_updater(SynaFirmware_truly);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	fwu->force_update = true;
	printk("synaptics@@@ make sure code comes here\n");
	if(input==0)
		retval=synaptics_fw_updater(SynaFirmware);
	else if(input==1)
		retval=synaptics_fw_updater(SynaFirmware_boe);
	else if(input==2)
		retval=synaptics_fw_updater(SynaFirmware_truly);
	printk("synaptics@@@ firmware update done\n");
	if (retval < 0) {
...
static ssize_t fwu_sysfs_read_config_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "device_config_id=0x%x,img_config_id=0x%x,device_fw_id=0x%x,img_fw_id=0x%x,touch_type=%d\n", g_device_config_id,g_image_config_id,g_device_fw_id,g_image_fw_id,synaptics_touch_type);
}
