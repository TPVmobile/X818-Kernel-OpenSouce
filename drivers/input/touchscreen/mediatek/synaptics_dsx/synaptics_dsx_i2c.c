...
/*TPV,Eric Lin,S2,20160317{*/
/*Enhancing TP boot/resume time*/
#if 0
#define DELAY_S7300_BOOT_READY  160
#define DELAY_S7300_RESET       20
#define DELAY_S7300_RESET_READY 90
#else
#define DELAY_S7300_BOOT_READY  10 //10~20
#define DELAY_S7300_RESET       10 //10~20
#define DELAY_S7300_RESET_READY 90 //>70
#endif
/*TPV,Eric Lin,S2,20160317}*/
...
//touch type
// 0:i2c addr=0x20,1:i2c addr=0x70,2:i2c addr=0x4b
unsigned char synaptics_touch_type=0;
...
//mli]
// for DMA accessing
static unsigned char  *gpwDMABuf_va;
static unsigned int gpwDMABuf_pa ;
static unsigned char  *gprDMABuf_va;
static unsigned int gprDMABuf_pa;
...
static ssize_t synaptics_rmi4_wake_stylus_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);

static ssize_t synaptics_rmi4_cal_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t synaptics_rmi4_suspend_cal_show(struct device *dev,
		struct device_attribute *attr, char *buf);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(suspend, S_IRUGO,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(wake_gesture, (S_IRUGO),
			synaptics_rmi4_wake_gesture_show,
			synaptics_rmi4_wake_gesture_store),
	__ATTR(wake_stylus, (S_IRUGO),
			synaptics_rmi4_show_error,
			synaptics_rmi4_wake_stylus_store),
	__ATTR(suspend_cal, (S_IRUGO),
			synaptics_rmi4_suspend_cal_show,
			NULL),
	__ATTR(cal, (S_IRUGO),
			synaptics_rmi4_cal_show,
			NULL),
};
		...
		struct device_attribute *attr, char *buf)
{
    ...
    unsigned char rec_data2;
    unsigned short addr = 0x012E;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			...
			sizeof(rec_data));
	if (retval < 0){
        printk("synaptics_rmi4_cal_show  >>>>");
		return 0;
    }
	...
	return snprintf(buf, PAGE_SIZE, "result 0x%x\n", rec_data2);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data,
			fhandler->full_addr.ctrl_base+11,
			&command,
			sizeof(command));

	return count;
}
static int synaptics_i2c_test(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for(i=0;i<10;i++)
	{
		retval = i2c_master_send(rmi4_data->i2c_client, buf, PAGE_SELECT_LEN);
	...
	rmi4_data->i2c_client->addr=0x70;
	for(i=0;i<10;i++)
	{
		retval = i2c_master_send(rmi4_data->i2c_client, buf, PAGE_SELECT_LEN);
		if(retval==PAGE_SELECT_LEN)
		{
			synaptics_touch_type=1;
			return 1;
		}
		msleep(20);
	}
	rmi4_data->i2c_client->addr=0x4b;
	for(i=0;i<10;i++)
	{
		retval = i2c_master_send(rmi4_data->i2c_client, buf, PAGE_SELECT_LEN);
		if(retval==PAGE_SELECT_LEN)
		{
			synaptics_touch_type=2;
			return 1;
		}
		msleep(20);
	}
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if(!gprDMABuf_va){
		...
		gprDMABuf_va = (unsigned char *)dma_alloc_coherent(&rmi4_data->input_dev->dev, 1024, (dma_addr_t *)&gprDMABuf_pa, GFP_KERNEL);
		if(!gprDMABuf_va){
	...
	read_msg[0].flags = (rmi4_data->i2c_client->flags& I2C_M_TEN) ;
	read_msg[0].len = 1;
	read_msg[0].buf = &buf;
	read_msg[0].timing = 400;
	read_msg[0].ext_flag = rmi4_data->i2c_client->ext_flag;
	...
	for (ii = 1; ii <= total; ii++) {
		...
		read_msg[ii].flags = (rmi4_data->i2c_client->flags& I2C_M_TEN) | I2C_M_RD;
		read_msg[ii].len = (ii == total) ? last : I2C_DMA_LIMIT;
		read_msg[ii].buf = (unsigned char *)(uintptr_t)gprDMABuf_pa + I2C_DMA_LIMIT * (ii - 1);
		read_msg[ii].ext_flag = (rmi4_data->i2c_client->ext_flag  | I2C_DMA_FLAG);
...
exit:
        if(gprDMABuf_va){
                dma_free_coherent(&(rmi4_data->input_dev->dev), 1024, gprDMABuf_va, gprDMABuf_pa);
                gprDMABuf_va = NULL;
                gprDMABuf_pa = 0;
        }
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	struct i2c_msg msg[] = {
		{
			...
			.flags = rmi4_data->i2c_client->flags & I2C_M_TEN,
			.len = length + 1,
			.buf = (unsigned char *)(uintptr_t)gpwDMABuf_pa,
			.ext_flag=(rmi4_data->i2c_client->ext_flag|I2C_DMA_FLAG),
			.timing = 400,
		}
	};
	mutex_lock(&(rmi4_data->rmi4_io_ctrl_mutex));
	if(!gpwDMABuf_va){
		//rmi4_data->input_dev->dev.coherent_dma_mask = DMA_BIT_MASK(32);
		gpwDMABuf_va = (unsigned char *)dma_alloc_coherent(&(rmi4_data->input_dev->dev), 6144, (dma_addr_t *)&gpwDMABuf_pa, GFP_KERNEL);
		if(!gpwDMABuf_va){
			printk("[Error] Allocate DMA I2C Buffer failed!\n");
        		}
	}
	buf_va = gpwDMABuf_va;
	msg[0].buf=(unsigned char *)(uintptr_t)gpwDMABuf_pa;
...
exit:
	...
	if(gpwDMABuf_va){
                dma_free_coherent(&(rmi4_data->input_dev->dev), 6144, gpwDMABuf_va, gpwDMABuf_pa);
                gpwDMABuf_va = NULL;
                gpwDMABuf_pa = 0;
        }
	mutex_unlock(&(rmi4_data->rmi4_io_ctrl_mutex));
	...
	if (enable) {
		...
		if (!rmi4_data->irq_enabled) {
            ...
            if(touch_irq == 0) {
                ...
                dev_err(&rmi4_data->i2c_client->dev, "synaptics_rmi4_irq_enable %d \n", touch_irq);
                ret = request_irq(touch_irq, (irq_handler_t)tpd_eint_handler, IRQF_TRIGGER_NONE, "synaptics_dsx_eint", NULL);
                if (ret > 0) {
                    dev_err(&rmi4_data->i2c_client->dev, "synaptics int register failed %d \n", touch_irq);
                } else {
                    dev_err(&rmi4_data->i2c_client->dev, "synaptics int register finished %d\n", touch_irq);
                }
		    } else {
		...
		const struct i2c_device_id *dev_id)
{
	...
	retval=synaptics_i2c_test(rmi4_data);
	if(retval<0)
	{
		dev_err(&client->dev,
				"%s: Failed to test i2c\n",
				__func__);
...
static void synaptics_rmi4_suspend(struct device *dev)
{
    ...
    // sf.xudong add for reduce power consumption
    GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);
